...
\item
  Connect the custom hearing aid output hardware class to the desired
...
You have now the frequency response of your setup in a struct in octave.
%
You can use all the tools available in octave to inspect and alter the
measured frequency response in this struct before using it to
calibrate the MHA.
%
We provide the octave function \texttt{plot\_fresponse} that you can use
...
by changing the correctionsdB vector in the frequency response struct in octave.
...
%
The octave function \texttt{fresponse\_to\_cfg} can be used to convert the
...
\texttt{peak} and for the \texttt{fir} configuration variables of the
\mha{} plugin \texttt{transducers}.

You can take these values and insert them into your configuration file
into the \texttt{transducers} configuration, remove the elipses
(\texttt{...}) printed, and remove superfluous semicolons.
%
...
When measuring the output levels for broadband noises, the measured
level cannot be predicted.
