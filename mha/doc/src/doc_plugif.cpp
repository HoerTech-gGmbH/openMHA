...
A MHA plugin is the signal processing unit, usually an algorithm. MHA
plugins can be combined into processing chains. One of the configured
chains can be selected for output which allowes direct comparison of
...
\li mha_wave_t * process(\ref mha_wave_t *): pure waveform processing
\li mha_spec_t * process(\ref mha_spec_t *): pure spectral processing
...
The exception should be of type MHAError. Exceptions of this type are caught by the MHAPLUGIN_CALLBACKS macro for further error Reporting.
