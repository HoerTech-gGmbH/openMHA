...
plugin is \emph{mha/examples/01-dynamic-compression/example\_dc.cfg}.
...
bands back into wide-band audio channels. For that, it needs the desired number of output channels.
\begin{verbatim}
...
while using the JACK server as an audio backend. While in the previous section
...
\end{verbatim}
%
As in the previous section, it is assumed, that the current working directory is the main \mha{} directory, and the environment variables are configured
