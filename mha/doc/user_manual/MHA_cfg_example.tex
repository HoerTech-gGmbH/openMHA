...
in \verb!prerelease_dynamiccompression.cfg!.
%
...
plugin is \verb!example_dc_live.cfg!.
...
in this case will be Jack audio server:
\begin{verbatim}
mhalib = transducers
iolib = MHAIOJack
\end{verbatim}
...
listed below, see \verb!prerelease_dynamiccompression.cfg!):
\begin{verbatim}
...
file \verb!example_dc_live.cfg!. Now we created a complete \mha{} hearing aid
...
%srate = 48000
%\end{verbatim}
We have also configured in the previous section which libraries to use 
(plugins and the IO backend, which is JACK).
%
It is assumed, that the current working directory is the main \mha{} directory, and the 
