...
acPooling_wave_config::acPooling_wave_config(algo_comm_t &ac, const mhaconfig_t in_cfg, acPooling_wave *_pooling):
    ...
    p(ac, _pooling->pool_name.data.c_str(), _pooling->numsamples.data, 1, true),
    p_max(ac, _pooling->max_pool_ind_name.data.c_str(), (_pooling->numsamples.data - 1) / 2),
    like_ratio(ac, _pooling->like_ratio_name.data.c_str(), 0),
    pooling_ind(0),
...
mha_wave_t *acPooling_wave_config::process(mha_wave_t *wave)
{
    ...
    like_ratio.data = p(max_ind, 0) / mean_p;

    if ( like_ratio.data >= up_thresh )
        p_max.data = max_ind;
    else if (like_ratio.data >= low_thresh) {
        // If the new maximum within the neighbourhood lyes
        if (neigh == -1)
            p_max.data = max_ind;
        else if (abs(max_ind - p_max.data) <= neigh)
            p_max.data = max_ind;
    }
 ...
 */
MHAPLUGIN_CALLBACKS(acPooling_wave,wave,wave)
...
MHAPLUGIN_DOCUMENTATION(
        "feedback",
        "This plugin computes an average over several consecutive frames using several different approaches (max, sum, mean, ...). Subsequently, the maximum of the average frame is delivered as well. The plugin receives the frames through the ac space and deliveres the averaged frame as well its maximum to the ac space."
        )
