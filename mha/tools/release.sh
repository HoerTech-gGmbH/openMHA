...
echo "file ReleaseProtocol.md, 'Release procedure' step 4, and"
echo "sections 'test_mhaioalsa.m and other automated live tests' and 'Run gain_live"
echo "example, dynamic compressor live example'? [yes|no]"
ask_yes_no;

echo "Was everything as expected? (refer to"
echo "file ReleaseProtocol.md for expected behaviour) [yes/no]"
ask_yes_no;
...
echo "Push branch main with new release to github [yes/no]?"
if $dry_run
then
...
echo "The new release state should soon be built by Github Actions. When this finishes without"
echo "errors, then "
echo " 1) check the documents generated by the Github Actions Job again,"
echo " 2) check the windows, mac and linux installers on test machines,"
echo " 3) on linux, compile example 21 plugin as described and test it."
