...
function test_adm
  ...
  inwav = '../Audiofiles/subject1_ref_Elko1995_ADM_in.wav';
  outwav = 'OUT.wav';
  unittest_teardown(@delete, [outwav]);
  fclose(fopen(outwav, 'w'));

  %% Test configuration
  dir='../../reference_algorithms/Elko1995_ADM/';
  cfg='Elko1995_ADM.cfg';

  %% Load input signal and reference output from disk
  [xAlgo,fs]=audioread(inwav);
  refyAlgo=audioread('../Audiofiles/subject1_ref_Elko1995_ADM_out.wav');
