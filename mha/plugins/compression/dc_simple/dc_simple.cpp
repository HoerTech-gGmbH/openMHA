...
// Copyright © 2007 2008 2009 2010 2013 2014 2015 2017 HörTech gGmbH
//
...
MHAPLUGIN_DOCUMENTATION(dc_simple,
    "level compression",
    ...
    "\n"
    "If spectral processing is used, the input level ($x$-axis of the input-output function)\n"
    "is determined by an attack- and release-filter of the short time RMS level $L_{st}$, which\n"
    "is calculated as described in the documentation of the 'fftfbpow' plugin or in equation 8 of \\citet{Grimm2006}.\n"
    "The attack filter is a first order low pass filter. The release filter is a maximum tracker, i.e.\n"
    "\\begin{eqnarray}\n"
    ...
    "\\end{eqnarray}\n"
    "The input level is divided into three sections. In each section the input level $L_{in}$ is transformed linearly into a gain $G$ on a log-log scale:\n"
    "$G_{dB} = (m-1) L_{in} + n$, where $m$ is the slope of the input-output function, and $n$ is an offset.\n"
    "Between expansion threshold and limiter threshold, $m$ and $n$ are given by the gain at 50 and 80 dB.\n"
    "In the section below the expansion threshold, $m$ is the expansion slope, and above the limiter threshold, $m$ is zero.\n"
    ...
    "bands).\n"
    "\n"
    "\\MHAfigure{Input-output function of one channel in the {\\em dc\\_simple} dynamic compression algorithm.}{dc_simple_in_out}\n"
    "\n"
