...
#include "ci_auralization_cis.hh"
      ...
      m_electrodes_cfg(m_electrodes)
{
    unsigned int phase_duration_samples =
        std::max(int(round(phase_duration_cfg * Ci_auralization_cis->input_cfg().srate)), 1);
    unsigned int interphase_gap_samples =
        round(interphase_gap_cfg * Ci_auralization_cis->input_cfg().srate);
    ...
    if (Ci_auralization_cis->input_cfg().fragsize < pulse_duration_samples)
    {
        throw MHA_Error(__FILE__, __LINE__,
                        ...
                        Ci_auralization_cis->input_cfg().fragsize);
    }
}
                                              ...
                                              MHA_AC::waveform_t *stimulation_signal_ac)
{
    ...
    for (unsigned int n = 0; n < n_electrodes_cfg; n++)
    {
        ...
        for (unsigned int m = 0; m < m_electrodes_cfg; m++)
        {
            ...
            {
                // If threshold level and maximum comfortable level are both
                // scalars:
                if (threshold_level_cfg.size() == 1 && maximum_comfortable_level_cfg.size() == 1)
                {
                    // Remap the current units to compressed electrical
                    // magnitude:
                ...
                // Else, if threshold level is a scalar and maximum comfortable
                // level is a vector:
                else if (threshold_level_cfg.size() == 1 && maximum_comfortable_level_cfg.size() > 1)
                {
                    // Remap the current units to compressed electrical
                    ...
                    value(stimulation_sequence, n, m) =
                        (value(stimulation_sequence, n, m) - threshold_level_cfg.at(0))/
                        (maximum_comfortable_level_cfg.at(m) - threshold_level_cfg.at(0));
                }
                // Else, if threshold level is a vector and maximum comfortable
                // level is a scalar:
                else if (threshold_level_cfg.size() > 1 && maximum_comfortable_level_cfg.size() == 1)
                {
                    // Remap the current units to compressed electrical
                    ...
                    value(stimulation_sequence, n, m) =
                        ...
                        (maximum_comfortable_level_cfg.at(0) - threshold_level_cfg.at(m));
                }
                // Else, if threshold level and maximum comfortable level are
                // both vectors:
                else if (threshold_level_cfg.size() > 1 && maximum_comfortable_level_cfg.size() > 1)
                {
                    // Remap the current units to compressed electrical
                    ...
                    value(stimulation_sequence, n, m) =
                        (value(stimulation_sequence, n, m) - threshold_level_cfg.at(m))/
                        (maximum_comfortable_level_cfg.at(m) - threshold_level_cfg.at(m));
                }
            }
        }
    }
...
Ci_auralization_cis::Ci_auralization_cis(algo_comm_t &ac,
    ...
    : MHAPlugin::plugin_t<Ci_auralization_cis_cfg>("CI auralization (CIS)",
                                                   ac),
      algo_name(algo_name),
      ...
      stimulation_signal_ac(nullptr)
{
    ...
    patchbay.connect(&ac_name.writeaccess, this, &Ci_auralization_cis::update_cfg);
    patchbay.connect(&compression_coefficient.writeaccess, this, &Ci_auralization_cis::update_cfg);
    patchbay.connect(&base_level.writeaccess, this, &Ci_auralization_cis::update_cfg);
    patchbay.connect(&saturation_level.writeaccess, this, &Ci_auralization_cis::update_cfg);
    patchbay.connect(&threshold_level.writeaccess, this, &Ci_auralization_cis::update_cfg);
    patchbay.connect(&maximum_comfortable_level.writeaccess, this, &Ci_auralization_cis::update_cfg);
    patchbay.connect(&electrode_distance.writeaccess, this, &Ci_auralization_cis::update_cfg);
    patchbay.connect(&lambda.writeaccess, this, &Ci_auralization_cis::update_cfg);
    patchbay.connect(&phase_duration.writeaccess, this, &Ci_auralization_cis::update_cfg);
    patchbay.connect(&interphase_gap.writeaccess, this, &Ci_auralization_cis::update_cfg);
    patchbay.connect(&phase_order.writeaccess, this, &Ci_auralization_cis::update_cfg);
}
...
void Ci_auralization_cis::release()
{
...
void Ci_auralization_cis::update_cfg()
{
    if (is_prepared())
    {
        push_config(new Ci_auralization_cis_cfg(ac_name.data,
...
MHAPLUGIN_CALLBACKS(ci_auralization_cis, Ci_auralization_cis, wave, wave)

MHAPLUGIN_DOCUMENTATION(ci_auralization_cis, "ci-vocoder",
"This plugin generates an auralized audio signal from the specified "
"AC variable, using a stimulation strategy similar to a typical CIS "
"(continuous interleaved sampling) coding strategy with 12 channels. It "
