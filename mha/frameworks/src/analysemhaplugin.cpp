...
// Copyright © 2006 2007 2009 2011 2013 2016 HörTech gGmbH
//
...
int main(int argc,char** argv)
{
    try{
        ...
        std::cout << "Categories:\n" << MHAParser::StrCnv::val2str(load.get_categories()) << std::endl << std::endl;
        ...
        for( indom = 0; indom < MHA_DOMAIN_MAX; indom++ ) 
            for( outdom = 0; outdom < MHA_DOMAIN_MAX; outdom++ ) 
                if( load.has_process(indom,outdom) )
                    std::cout << "  " << strdom(indom) << " to " << strdom(outdom) << std::endl;
        std::cout << std::endl;
        ...
        try{
            ...
            std::cout << "Prepare for waveform processing failed with message:" << std::endl;
        ...
        try{
            ...
            std::cout << "Successfully prepared for spectrum processing." << std::endl << std::endl;
            print_ac(ac,"after prepare");
            load.release();
            print_ac(ac,"after release");
        }
        catch( MHA_Error& e ){
            std::cout << "Prepare for spectrum processing failed with message:" << std::endl;
            std::cout << e.what() << std::endl << std::endl;
        }
