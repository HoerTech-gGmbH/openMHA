...
rem Set preset name, fragment size, and sampling rate for JACK and openMHA, 
rem respectively:
...
rem Seeds for randomization of stimulation order (same value for left and 
rem right: binaural link on; different values for left and right: binaural link 
rem off):
set ELECTRODOGRAM_RANDOMIZATION_SEED_LEFT=1
set ELECTRODOGRAM_RANDOMIZATION_SEED_RIGHT=1
...
rem Input and output channels to be used by JACK:
set JACK_INPUT=[system:capture_1 system:capture_2]
...
cls
start /wait /b mha --interactive ?read:cfg/vocoder_ace_binaural_realtime.cfg cmd=start
pause
