...
% acoustic hearing (ACE strategy), plotting the results, and displaying
...
fc = 713;       % cutoff frequency / Hz (lowest CI auralization frequency)
...
hold off;
ylabel('Magnitude / dB');
title({'Residual Acoustic Hearing Filter (ACE)', ''});
subplot(2, 1, 2);
hold on;
plot([fc fc], ylim, 'r--');
yl = ylim;
...
fprintf('Residual acoustic hearing filter for ACE (Linux):\n\n');
fprintf(['export ACOUSTIC_IIRFILTER_B="[' sB ']"\n']);
fprintf(['export ACOUSTIC_IIRFILTER_A="[' sA ']"\n\n\n']);
fprintf('\nResidual acoustic hearing filter for ACE (Windows):\n\n');
