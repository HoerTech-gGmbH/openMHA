...
% auralization (CIS strategy), plotting the results, and displaying them on
...
% compute_synthesis_center_frequencies_cis.m.
%
...
% fc = [359 548 679 945 1484 2234 3315 4592 6536 9736 12847 NaN]

srate = 48000;  % sampling rate / Hz
fc = [359 548 679 945 1484 2234 3315 4592 6536 9736 12847 15641];  % center frequencies / Hz
n = 3;          % filter order
...
ylim([-40 0]);
ylabel('Magnitude / dB');
title({'Synthesis Filterbank (CIS)', ''});

% Display the results:
expression = '(?<=e[+-])0';
fprintf('Synthesis filterbank for CIS (Linux):\n\n');
fprintf('export GTFB_ANALYZER_SYNTHESIS_ORDER=%u\n', n);
for k = 1:length(fc)
    ...
    fprintf(['export GTFB_ANALYZER_SYNTHESIS%u_COEFF="[(' sCoefficient ')]"\n'], k-1);
    fprintf(['export GTFB_ANALYZER_SYNTHESIS%u_NORM_PHASE="[' sNormalization_factor ']"\n'], k-1);
end
fprintf('\n\n\n');
fprintf('Synthesis filterbank for CIS (Windows):\n\n');
fprintf('set GTFB_ANALYZER_SYNTHESIS_ORDER=%u\n', n);
for k = 1:length(fc)
    coefficient = analyzer.filters(k).coefficient;
    sCoefficient = strtrim(sprintf('%g%+gi ', real(coefficient), imag(coefficient)));
    sCoefficient = regexprep(sCoefficient, expression, '');
    ...
    sNormalization_factor = strtrim(sprintf('%g ', normalization_factor));
    sNormalization_factor = regexprep(sNormalization_factor, expression, '');
    fprintf(['set GTFB_ANALYZER_SYNTHESIS%u_COEFF=[(' sCoefficient ')]\n'], k-1);
    fprintf(['set GTFB_ANALYZER_SYNTHESIS%u_NORM_PHASE=[' sNormalization_factor ']\n'], k-1);
end
fprintf('\n');
