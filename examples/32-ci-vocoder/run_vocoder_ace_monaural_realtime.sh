...
export ELECTRODOGRAM_RANDOMIZATION_SEED=1
...
# Input and output channels to be used by JACK (if two output channels are 
# specified for monaural processing, the openMHA output is presented 
# identically on both channels):
export JACK_INPUT="[system:capture_1]"
...
clear
mha --interactive ?read:cfg/vocoder_ace_monaural_realtime.cfg cmd=start
